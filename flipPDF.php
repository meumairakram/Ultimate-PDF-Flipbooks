...
****/
 ...
 if(!function_exists('upfb_check_uploading_directory')) {
    ...
    // check for default_option


    function upfb_check_uploading_directory() {

        $show_notice = get_option('upfb_admin_notice');

        if($show_notice == false) {
            update_option('upfb_admin_notice','0');
        }
        ...
        if(!chmod(UPFB_UPLOAD_PATH,0755) || !chmod(wp_upload_dir()['basedir'],0755)) {
            update_option('upfb_admin_notice','1');
        }
    ...
    if(isset($upload_nonce) && wp_verify_nonce($upload_nonce,'2635') && current_user_can('edit_post',$post_id) && isset($_FILES['upfb_book_attachment'])) {
        ...
        if(empty($errors)==true){
           ...
           if(file_exists(UPFB_UPLOAD_PATH.'/'.$folder_name)) {
                ...
                $file_perm =  chmod(UPFB_UPLOAD_PATH.'/'.$folder_name,0755);
                $file_perm1 = chmod(UPFB_UPLOAD_PATH.'/'.$folder_name.'/upload.php',0755);

                if(!$file_perm || !$file_perm1) {
                    update_option('upfb_admin_notice','1');
                }
                ...
                if(!is_executable(UPFB_UPLOAD_PATH.'/'.$folder_name.'/upload.php')) {
                    if(!chmod(UPFB_UPLOAD_PATH.'/'.$folder_name.'/upload.php',0755)){
                        update_option('upfb_admin_notice','1');
                    }
                }
...
if(!function_exists('upfb_show_admin_error')){
    function upfb_show_admin_error() {
?>

        <div class="notice error my-acf-notice is-dismissible" >
        <p><strong>Ultimate PDF Flipbooks</strong><?php _e( ' is unable to adjust folder permissions and might cause errors in Flipbooks Loading, Please contact your Hosting Provider or the Plugin Author for further information, about this error. ', 'my-text-domain' ); ?></p>
        </div>
...
if(get_option('upfb_admin_notice') == '1') {
    add_action('admin_notices','upfb_show_admin_error');
}        
